...
function convertToEvidenceItem(
  record: EvidenceRecord,
): import("../services/AIAnalysisService.js").EvidenceItem {
  return {
    ...record,
    evidenceId: record.id,
    contentType: record.metadata?.mimeType?.startsWith("image/")
      ? "image"
      : record.metadata?.mimeType?.startsWith("video/")
        ? "video"
        : record.metadata?.mimeType?.startsWith("audio/")
          ? "audio"
          : "document",
...
const detectPatternsSchema = z.object({
  ...
  patternTypes: z
    .array(z.enum(["temporal", "spatial", "behavioral", "content"]))
    .default(["temporal", "content"]),
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      // Perform AI analysis
      const analysis = await aiAnalysisService.analyzeEvidence(
        evidence,
        analysisRequest,
      );
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      // Detect patterns
      const patterns = await aiAnalysisService.detectPatterns(
        evidenceItems,
        validatedData.patternTypes,
      );
    ...
    try {
      ...
      const evidenceItems = validatedData.evidenceIds.map((evidenceId) => {
        const mockRecord: EvidenceRecord = {
          ...
          metadata: { mimeType: "application/pdf" },
          createdAt: new Date(
            Date.now() - (evidenceId % 30) * 24 * 60 * 60 * 1000,
          ), // Deterministic dates based on ID
          retentionPeriod: 7 * 365 * 24 * 60 * 60 * 1000,
        };
        return convertToEvidenceItem(mockRecord);
      });
    ...
    try {
      ...
      if (evidenceItems.length < 2) {
        return res.status(400).json({
          error:
            "At least 2 evidence items required for cross-reference analysis",
        });
      }

      // Perform cross-reference analysis
      const crossReference =
        await aiAnalysisService.crossReferenceEvidence(evidenceItems);
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      const capabilities = {
        ...
        patternTypes: ["temporal", "spatial", "behavioral", "content"],
        legalFeatures: [
