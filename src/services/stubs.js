...
// MCP Service - Proxy to ChittyMCP Worker
export async function handleMCP(context) {
  const { request, env } = context;

  // If CHITTYMCP service binding is available, proxy to it
  if (env.CHITTYMCP) {
    try {
      return await env.CHITTYMCP.fetch(request);
    } catch (error) {
      console.error("ChittyMCP service binding error:", error);
      // Fall through to stub response
    }
  }

  // Fallback stub response if service binding unavailable
  ...
  if (path === "/health") {
    return new Response(
      JSON.stringify({
        service: "mcp",
        status: "degraded",
        message: "ChittyMCP service binding unavailable, using stub",
  ...
  return new Response(
    JSON.stringify({
      message: "MCP service stub - ChittyMCP service binding unavailable",
      available: ["/health"],
      note: "Configure CHITTYMCP service binding to enable full functionality",
    }),
    {
      status: 503,
      headers: { "content-type": "application/json" },
  ...
  if (path === "/api/v1/services" || path === "/v1/services") {
    const services = [
      // AI Infrastructure
      {
        ...
        status: "active",
        version: "1.0.0",
        endpoints: ["/health", "/v1/chat/completions", "/v1/embeddings"],
      },
      {
        name: "langchain",
        host: "langchain.chitty.cc",
        status: "active",
        version: "1.0.0",
        endpoints: ["/health", "/agents", "/chains"],
      },
      {
        name: "mcp-agents",
        host: "mcp.chitty.cc",
      ...
      {
        name: "agents",
        host: "agents.chitty.cc",
        status: "active",
        version: "1.0.0",
        endpoints: ["/health", "/create", "/execute"],
      },
      {
        name: "unified",
        host: "unified.chitty.cc",
        status: "active",
        version: "1.0.0",
      ...
      },
      {
        ...
        description:
          "OAuth-enabled OpenAI-compatible API using Google Gemini backend with ChittyAuth",
      },
      ...
      {
        name: "beacon",
        host: "beacon.chitty.cc",
        status: "active",
        version: "1.0.0",
        endpoints: ["/health", "/track", "/dashboard"],
      },
      ...
      // Identity & Auth
      {
        ...
        status: "active",
        version: "2.1.0",
        endpoints: ["/health", "/generate", "/verify"],
      },
      {
        name: "auth",
        host: "auth.chitty.cc",
        status: "active",
        version: "1.0.0",
        endpoints: ["/health", "/login", "/logout", "/token"],
      },

      // Service Mesh
      {
        name: "registry",
        host: "registry.chitty.cc",
        status: "active",
        version: "1.0.0",
        endpoints: ["/health", "/register", "/api/v1/services"],
      },
      {
        name: "canon",
        host: "canon.chitty.cc",
        status: "active",
        version: "1.0.0",
      ...
      {
        name: "schema",
        host: "schema.chitty.cc",
        status: "active",
        version: "1.0.0",
        endpoints: ["/health", "/schemas", "/validate"],
      },
      {
        name: "vectorize",
        host: "vectorize.chitty.cc",
        status: "development",
        version: "0.5.0",
        endpoints: ["/health", "/embed", "/search"],
      },
      {
        name: "hyperdrive",
        host: "hyperdrive.chitty.cc",
        status: "development",
        version: "0.5.0",
        endpoints: ["/health"],
      },
      ...
      {
        name: "viewer",
        host: "viewer.chitty.cc",
        status: "development",
        version: "0.5.0",
        endpoints: ["/health", "/view"],
      },

      // Infrastructure
      {
        name: "audit",
        host: "audit.chitty.cc",
        status: "development",
        version: "0.5.0",
        endpoints: ["/health", "/log"],
      },
      {
        name: "assets",
        host: "assets.chitty.cc",
        status: "development",
        version: "0.5.0",
        endpoints: ["/health"],
      },
      ...
      {
        name: "staging",
        host: "staging.chitty.cc",
        status: "active",
        version: "1.0.0",
        endpoints: ["/health", "/ai", "/sync", "/beacon"],
      },
      {
        name: "staging-ai",
        host: "staging-ai.chitty.cc",
        status: "active",
        version: "1.0.0",
        endpoints: ["/health"],
      },
...
// Import real Sync service - Platform Integration Hub
export { handleSync } from "./sync.js";
