...
Format as a comprehensive legal research memo.`;
  ...
  async analyzeDocument(params) {
    ...
    const analysisPrompt = `Analyze the following legal document:
...
Provide detailed analysis with specific citations and strategic recommendations.`;
  ...
  /**
   * ChittyCases MCP Tools - Cook County Data Integration
   */
  async verifyCaseNumber(params) {
    const { case_number, party_names } = params;
    ...
    try {
      // Connect to ChittyCases scraped database
      const response = await fetch(`${this.env.CHITTYCASES_ENDPOINT || 'https://chittycases.pages.dev'}/api/cases/verify`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${this.env.CHITTYCASES_TOKEN || ''}`
        },
        body: JSON.stringify({ case_number, party_names })
      });

      if (!response.ok) {
        throw new Error(`ChittyCases API error: ${response.status}`);
      }
      ...
      return {
        isValid: false,
        message: `ChittyCases verification failed: ${error.message}`,
        case_number,
        timestamp: new Date().toISOString()
      };
    }
  ...
  async searchCases(params) {
    const { party_name, attorney_name, date_from, date_to } = params;

    try {
      const response = await fetch(`${this.env.CHITTYCASES_ENDPOINT || 'https://chittycases.pages.dev'}/api/cases/search`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${this.env.CHITTYCASES_TOKEN || ''}`
        },
        body: JSON.stringify({ party_name, attorney_name, date_from, date_to })
      });
      ...
      return {
        error: `ChittyCases search failed: ${error.message}`,
        results: [],
        timestamp: new Date().toISOString()
      };
    }
  ...
  async getCourtCalendar(case_number) {
    if (!case_number) {
      throw new Error("Case number is required");
    }

    try {
      const response = await fetch(`${this.env.CHITTYCASES_ENDPOINT || 'https://chittycases.pages.dev'}/api/cases/${case_number}/calendar`, {
        headers: {
          'Authorization': `Bearer ${this.env.CHITTYCASES_TOKEN || ''}`
        }
      });
      ...
      return {
        error: `ChittyCases calendar fetch failed: ${error.message}`,
        events: [],
        case_number,
        timestamp: new Date().toISOString()
      };
  ...
  async lookupPropertyPIN(params) {
    const { pin, include_tax_history } = params;

    if (!pin) {
      throw new Error("Property PIN is required");
    }

    try {
      const response = await fetch(`${this.env.CHITTYCASES_ENDPOINT || 'https://chittycases.pages.dev'}/api/property/${pin}`, {
        headers: {
          'Authorization': `Bearer ${this.env.CHITTYCASES_TOKEN || ''}`
        }
      });
      ...
      return {
        ...
        address: null,
        unit: null,
        error: `ChittyCases property lookup failed: ${error.message}`,
        timestamp: new Date().toISOString()
      };
    }
  }

  async checkFilingCompliance(params) {
    const { case_number, document_type, metadata } = params;

    if (!case_number || !document_type) {
      throw new Error("Case number and document type are required");
    }

    try {
      const response = await fetch(`${this.env.CHITTYCASES_ENDPOINT || 'https://chittycases.pages.dev'}/api/filing/compliance`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${this.env.CHITTYCASES_TOKEN || ''}`
        },
        body: JSON.stringify({ case_number, document_type, metadata })
      });
      ...
      return {
        ...
        case_number,
        document_type,
        issues: [`ChittyCases compliance check failed: ${error.message}`],
        recommendations: ['Verify ChittyCases service availability'],
        timestamp: new Date().toISOString()
      };
    }
  }
