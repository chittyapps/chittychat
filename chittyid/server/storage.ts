...
export interface IStorage {
  ...
  createVerificationRequest(
    data: InsertVerificationRequest,
  ): Promise<VerificationRequest>;
  getVerificationRequestsByBusiness(
    businessId: string,
  ): Promise<VerificationRequest[]>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getUserByChittyId(chittyId: string): Promise<User | undefined> {
    try {
      ...
      const [user] = await db
        .select()
        .from(users)
        .where(eq(users.chittyId, chittyId));
      console.log(
        `Found user:`,
        user ? `${user.email} (ChittyID: ${user.chittyId})` : "null",
      );
      return user;
    } catch (error) {
  ...
  async createUser(userData: {
    ...
    await chittyIdService.syncUserWithMothership(
      userId,
      chittyIdCode,
      userData,
    );
  ...
  async updateChittyIdTrustScore(
    id: string,
    trustScore: number,
    trustLevel: string,
  ): Promise<ChittyId> {
  ...
  async getVerificationRequestsByBusiness(
    businessId: string,
  ): Promise<VerificationRequest[]> {
    return await db
      .select()
